...
class ManufacturingYearNotesTests(ExcelExportServiceCompanyDataTests):
    """Тесты формирования доп. примечаний по году выпуска"""

    def test_get_additional_note_for_non_current_manufacturing_year(self):
        """Если год выпуска не текущий, должно добавляться примечание о несоответствии"""
        current_year = datetime.now().year
        non_current_year = current_year - 1
        self.insurance_request.manufacturing_year = str(non_current_year)

        result = self.service._get_additional_note_for_manufacturing_year(self.insurance_summary)

        self.assertEqual(result, self.service.MANUFACTURING_YEAR_ADDITIONAL_NOTE)

    def test_get_additional_note_for_current_manufacturing_year(self):
        """Если год выпуска текущий, доп. примечание не добавляется"""
        current_year = datetime.now().year
        self.insurance_request.manufacturing_year = str(current_year)

        result = self.service._get_additional_note_for_manufacturing_year(self.insurance_summary)

        self.assertIsNone(result)

    def test_get_additional_note_for_empty_manufacturing_year(self):
        """Если год выпуска пустой, должно добавляться примечание"""
    ...
    def test_get_additional_note_for_invalid_manufacturing_year(self):
        """Если год выпуска невалидный, должно добавляться примечание"""
        self.insurance_request.manufacturing_year = 'год неизвестен'
