...
class AssetStatusNotesTests(ExcelExportServiceCompanyDataTests):
    """Тесты формирования доп. примечаний по статусу имущества"""

    def test_get_additional_note_for_non_new_asset_status(self):
        """Если статус имущества не 'новое', должно добавляться примечание"""
        self.insurance_request.asset_status = 'б/у'
    ...
    def test_get_additional_note_for_new_asset_status(self):
        """Если статус имущества 'новое', доп. примечание не добавляется"""
    ...
    def test_get_additional_note_for_empty_asset_status(self):
        """Если статус имущества пустой, должно добавляться примечание"""
    ...
    def test_get_additional_note_for_new_asset_status_with_whitespace_and_case(self):
        """Пробелы/регистр не должны влиять на распознавание статуса 'новое'"""
        self.insurance_request.asset_status = '  НоВоЕ  '
    ...
    def test_build_export_notes_combines_offer_and_additional_notes(self):
        ...
        self.assertEqual(result, 'Специальные условия Доп. условие')

    def test_build_export_notes_uses_additional_note_when_offer_note_missing(self):
        """Если комментария страховщика нет, используется только доп. примечание"""
...
class IntegrationTests(ExcelExportServiceCompanyDataTests):
    ...
    def test_fill_company_data_passes_combined_additional_note_for_non_new_or_empty_asset_status(self):
        """В строки компании передаются оба системных комментария: по статусу имущества и по франшизе"""
        workbook = Workbook()
        ...
        self.assertEqual(
            ...
            (
                f"{self.service.ASSET_STATUS_ADDITIONAL_NOTE} "
                f"{self.service.FRANCHISE_APPROVAL_ADDITIONAL_NOTE}"
            )
        )

    def test_fill_company_data_passes_only_franchise_note_for_new_asset_status(self):
        """При статусе 'новое' передается только комментарий о согласовании франшизы"""
        workbook = Workbook()
        companies_data = {'ВСК': [self.offer3]}
        self.insurance_request.asset_status = 'новое'
