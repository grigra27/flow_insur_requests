...
class ExcelExportService:
    ...
    def _create_rate_formula(self, worksheet, row_num: int, column: str) -> None:
        ...
        Формула вычисляет тариф как долю (премия / страховая сумма); проценты даёт
        формат ячейки 0.00% из шаблона, поэтому умножать на 100 нельзя.
        ...
        try:
            ...
            # Формула: ЕСЛИ(И(страховая_сумма<>0, премия<>0), премия/страховая_сумма, "")
            formula = f'=IF(AND(C{row_num}<>0,{premium_column}{row_num}<>0),{premium_column}{row_num}/C{row_num},"")'
