...
def _parse_statistics_filters(request):
    """Парсинг и валидация фильтров периода (быстрый период или явные даты)."""
...
def _parse_company_analytics_filters(request):
    """Парсинг фильтров для аналитики по страховым компаниям: период, филиал, тип."""
...
def _build_company_analytics(filters):
    return build_analytics_insurance_companies_payload(
...
def analytics_insurance_companies(request):
    ...
    payload = _build_company_analytics(filters)
    context = {
        **payload,
        'available_filters': build_company_analytics_available_filters(
        ...
        'heatmap_titles': [('branch', 'СК × филиал'), ('insurance_type', 'СК × тип страхования')],
    }
...
def export_analytics_insurance_companies_widget(request):
    ...
    if filters['start_date'] and filters['end_date']:
        ...
        period_display = 'Всё время'
    scope_parts = [f"Период (дата создания свода): {period_display}"]
    if filters['branch']:
        scope_parts.append(f"Филиал: {filters['branch']}")
    if filters['insurance_type']:
        scope_parts.append(f"Тип страхования: {filters['insurance_type']}")
    ...
    def add_sheet(title, headers, rows, first=False):
        ...
        worksheet.append(['; '.join(scope_parts)])
    ...
    add_sheet('Сводка', ['Показатель', 'Значение'], [
        ['Сделок (акцептованных сводов)', kpi['total_deals']],
        ['Страховая сумма, 1-й год, ₽', _decimal_to_float(kpi['insured_sum_total'])],
        ['Премия, все годы, ₽', _decimal_to_float(kpi['premium_total'])],
        ['Средний тариф 1-го года, %', _decimal_to_float(kpi['avg_tariff_pct'])],
        ['СК-победителей', kpi['winners_count']],
        ['СК, участвовавших в сделках', kpi['participants_count']],
        ['Сопоставимых по цене сделок', kpi['comparable_deals']],
        ['Выбрана самая дешёвая, сделок', kpi['cheapest_deals']],
        ['Выбрана самая дешёвая, %', _decimal_to_float(kpi['cheapest_rate_pct'])],
    ], first=True)
    add_sheet('СК', [
        'СК', 'Сделок', 'Доля сделок, %', 'Страховая сумма, ₽', 'Доля страховой суммы, %',
        'Премия, ₽', 'Доля премии, %', 'Тариф 1-го года, %', 'Участвовала в сделках',
        'Доля выигрышей, %', 'Сопоставимых выигрышей', 'Из них самая дешёвая',
    ], [
        [
            row['company_name'], row['deals'], _decimal_to_float(row['deals_share_pct']),
            _decimal_to_float(row['insured_sum']), _decimal_to_float(row['insured_sum_share_pct']),
            _decimal_to_float(row['premium_total']), _decimal_to_float(row['premium_share_pct']),
            _decimal_to_float(row['tariff_pct']), row['offered'], _decimal_to_float(row['win_rate_pct']),
            row['comparable_wins'], row['cheapest_wins'],
        ]
        for row in payload['company_rows']
    ])
    add_sheet('По месяцам', ['Месяц', 'Сделок', 'Страховая сумма, ₽', 'Лидер по сделкам'], [
        [row['month_label'], row['deals'], _decimal_to_float(row['insured_sum']), row['leader']]
        for row in payload['monthly_rows']
    ])
    for key, title in (('branch', 'СК × филиал'), ('insurance_type', 'СК × тип')):
        heatmap = payload['heatmaps'][key]
        add_sheet(title, ['СК', *heatmap['columns'], 'Итого'], [
            [row['label'], *[cell['value'] for cell in row['cells']], row['total']] for row in heatmap['rows']
        ])
    ...
    response['Content-Disposition'] = f'attachment; filename="analytics_companies_{today}.xlsx"'
    return response
