...
- **Отчёт деплоя / SSL-скрипты — ✅ сделано (деплой `5a98a3e`, 2026-09-24).** Продление сертификатов всё время работало
  ...
  Из-за этого каждый деплой считал сертификаты отсутствующими и запускал `obtain-certificates-docker.sh`, чей провал
  маскировался `| tee`. Исправлено: пути от корня проекта (переопределяются `LETSENCRYPT_DIR`, `SSL_CHECK_LOG_FILE`),
  публичные `cert.pem`/`chain.pem` читаются через одноразовый контейнер certbot (`live/` — root 700), цепочка —
  `-untrusted chain.pem` + системный корень, статусы не обрывают скрипт. Вызов `ssl-cron-setup.sh` (требует root,
  дублирует certbot) убран из деплоя. Проверено на сервере от `deploy`: оба сертификата валидны, exit 0;
  без сертификатов — exit 2.
